  ...
  methods: {
    async submit() {
      if (this.$refs.form.validate()) {
        ...
        try {
          const { data } = await this.$axios.get(`/api/stats/service-alerts?admins=${newStatReq.admins}&startDate=${newStatReq.startDate}&endDate=${newStatReq.endDate}`)
    ...
    getValue({ service, total_alerts }) {
      this.values.push(total_alerts)
    ...
    savePDF() {
      const doc = new jsPDF()
      const usersText = []
      doc.text(`Alerts By Service for ${this.fromDate} - ${this.toDate}`, 10, 10)
      for (let i = 0, total = this.values.length; i < total; i++) usersText.push(`${this.labels[i]}: ${this.values[i]}`)
      doc.text(usersText, 20, 20)
      doc.save(`alerts-by-service_${new Date().toDateString()}.pdf`)
    }
  }
}
</script>
